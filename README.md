...
toUnixstampDate : YYYYMMDD 형식의 날짜를 tz에 맞는 Unixstamp number(UTC)로 변환합니다.
```javascript
...
toDayOfTheWeek : Unixstamp 날짜 정보에서 요일 정보를 추출합니다.
```javascript
...
getStartTimeOfEachDat : 선택 가능한 날짜 중 시작 시간과 입력받은 날로부터 며칠 후인지를 반환하는 함수
```javascript
...
getDayTimeTable : DayTimeTables를 얻기 위한 함수 (ex) 배열.map(getDayTimeTable) = DayTimeTables
```javascript
...
getTimeslots : DayTimeTable 중 timeslots 값을 얻기 위한 함수
```javascript
...
getDayTimetables : 모든 함수를 사용하여 요청의 응답값을 만드는 함수
```javascript
